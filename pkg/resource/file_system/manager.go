...
// +kubebuilder:rbac:groups=fsx.services.k8s.aws,resources=filesystems,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=fsx.services.k8s.aws,resources=filesystems/status,verbs=get;update;patch
...
func (rm *resourceManager) IsSynced(ctx context.Context, res acktypes.AWSResource) (bool, error) {
	...
	if r.ko.Status.Lifecycle == nil {
		return false, nil
	}
	lifecycleCandidates := []string{"AVAILABLE"}
	if !ackutil.InStrings(*r.ko.Status.Lifecycle, lifecycleCandidates) {
		return false, nil
	}
